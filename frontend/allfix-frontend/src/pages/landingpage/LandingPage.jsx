...
import {
  ...
  AppBar, Toolbar, IconButton, TextField, MenuItem, CssBaseline,
} from '@mui/material';
  ...
  return (
    <>
      ...
      <Box sx={{ width: '100%', overflowX: 'hidden' }}>
          ...
          <Toolbar sx={{ display: 'flex', justifyContent: 'space-between', px: { xs: 2, md: 5 }, minHeight: '64px' }}>
            <Box sx={{ display: 'flex', alignItems: 'center', gap: 1.5, ml: { xs: 0, md: 8, lg: 16 }, flex: { xs: '1 1 auto', md: 'none' } }}>
              <Box
                ...
                src="/ALLFIXLOGO.png"
                alt="AllFix.ph Logo"
                sx={{ width: { xs: 35, md: 45 }, height: { xs: 35, md: 45 }, objectFit: 'contain' }}
              />
            ...
            <Box sx={{ display: { xs: 'none', md: 'flex' }, gap: 0.5, mr: { lg: 4, md: 2 } }}>
              {navLinks.map((link) => (
                <Button
                  ...
                  sx={{ px: 1.5, py: 0.5, borderRadius: 1, fontSize: '0.75rem', fontWeight: 600, textTransform: 'none', color: isScrolled ? '#10355f' : 'rgba(255,255,255,0.9)', '&:hover': { backgroundColor: isScrolled ? 'rgba(16, 53, 95, 0.1)' : 'rgba(255,255,255,0.1)', color: isScrolled ? '#10355f' : 'white' } }}
                >
                  {link.label}
                </Button>
              ))}
              ...
              <Box sx={{ px: 2, py: 2, display: 'flex', flexDirection: 'column', gap: 0.5 }}>
                {navLinks.map((link) => (
                  <Button
                    ...
                    onClick={() => handleNavClick(link.href)}
                    fullWidth
                    sx={{ justifyContent: 'flex-start', px: 2, py: 1.5, borderRadius: 1, fontSize: '0.875rem', fontWeight: 500, textTransform: 'none', color: isScrolled ? '#10355f' : 'white', '&:hover': { backgroundColor: isScrolled ? 'rgba(16, 53, 95, 0.1)' : 'rgba(255,255,255,0.2)' } }}
                  >
                    {link.label}
                  </Button>
                ))}
            ...
            <Grid container spacing={4} alignItems="center" justifyContent="space-between" sx={{ width: '100%' }}>
              ...
              <Grid item xs={12} md={6} lg={7} sx={{ display: 'flex', flexDirection: 'column', justifyContent: 'center', gap: 3 }}>
                ...
                <Box sx={{ display: 'flex', flexDirection: 'row', gap: { xs: 3, md: 5 }, flexWrap: 'wrap' }}>
                  ...
                  <Box sx={{ display: 'flex', alignItems: 'center', gap: 2 }}>
                    <ShieldIcon sx={{ width: 28, height: 28, color: '#60a5fa', flexShrink: 0 }} />
                    <Box>
              ...
              <Grid item xs={12} md={5} sx={{ display: 'flex', alignItems: 'flex-start', justifyContent: { xs: 'center', md: 'flex-end' } }}>
                <Box sx={{ bgcolor: 'white', borderRadius: '16px', p: { xs: 2.5, md: 3 }, boxShadow: '0 20px 60px rgba(0, 0, 0, 0.2)', width: '100%', maxWidth: { xs: '98vw', sm: '480px' }, mx: { xs: 'auto', md: 0 }, ml: { md: 'auto' }, mt: { xs: 2, md: -4 } }}>
                  ...
                  <Box sx={{ mb: 1.5 }}>
                    ...
                    <TextField
                      ...
                      InputProps={{ startAdornment: <Typography sx={{ mr: 1, color: '#999', fontSize: '1rem' }}>✉</Typography> }}
                      sx={{ '& .MuiOutlinedInput-root': { borderRadius: '8px', fontSize: '0.85rem', '& fieldset': { borderColor: '#ddd' }, '&:hover fieldset': { borderColor: '#bbb' } } }}
                    />
                  </Box>
                  ...
                  <Box sx={{ mt: 1.5, textAlign: 'center' }}>
                    ...
                    <Button variant="contained" fullWidth href="/login" sx={{ bgcolor: '#fff', color: '#10355f', border: '2px solid #10355f', borderRadius: '50px', fontWeight: 900, fontSize: '0.9rem', py: 0.8, mt: 0.5, mb: 0.5, boxShadow: '0 2px 8px rgba(16,53,95,0.10)', textTransform: 'none', '&:hover': { bgcolor: '#10355f', color: '#fff', borderColor: '#0d264a' } }}>
                      Login
                    </Button>
                  </Box>
                </Box>
            ...
            <Box sx={{ width: '100%', mt: 2 }}>
              ...
              <Box sx={{ display: { xs: 'flex', md: 'none' }, flexDirection: 'column', alignItems: 'center', width: '100%' }}>
                <Box sx={{ display: 'flex', flexDirection: 'row', alignItems: 'center', width: '100%' }}>
                  ...
                  <Box id="services-scroll-row" sx={{ display: 'flex', gap: 2, overflowX: 'auto', scrollSnapType: 'x mandatory', px: 1, flex: 1, scrollBehavior: 'smooth', '&::-webkit-scrollbar': { display: 'none' } }}>
                ...
                <Box sx={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: 1, width: '100%', mt: 1, mb: 1, px: 1 }}>
                  {services.map((svc, idx) => (
                    <Button key={svc.brand} size="small" sx={{ minWidth: 0, px: 0.5, py: 0.8, fontSize: '0.75rem', fontWeight: 700, borderRadius: '10px', color: '#23406e', bgcolor: '#eaf2fc', whiteSpace: 'nowrap', boxShadow: 'none', border: '1.5px solid transparent', '&:hover': { bgcolor: '#dbeafe', color: '#23406e' } }} onClick={() => document.getElementById('services-scroll-row')?.scrollTo({ left: idx * 340, behavior: 'smooth' })}>
                      {svc.brand}
                    </Button>
                  ))}
                </Box>
        ...
        <Box id="how-it-works" sx={{ position: 'relative', zIndex: 10, bgcolor: '#eef4fd', py: { xs: 6, md: 16 }, px: { xs: 2, md: 10 }, minHeight: '100vh', display: 'flex', flexDirection: 'column', justifyContent: 'center' }}>
          <Container maxWidth="lg">
            ...
            <Box sx={{ display: 'flex', flexDirection: { xs: 'column', md: 'row' }, alignItems: 'flex-start', justifyContent: 'center', gap: { xs: 6, md: 2 }, position: 'relative', mb: 4 }}>
              ...
              {[
                { Icon: CheckCircleIcon, title: 'Choose a Service', desc: 'Select the type of work you need from our 9 specialized brands. Browse by category or search directly.', step: '1' },
                { Icon: DescriptionIcon,  title: 'Book a Service',   desc: 'Schedule your service instantly with just a few clicks. Fast, easy, and convenient.',                      step: '2' },
                { Icon: CheckCircleIcon, title: "Sit Back, It's Done", desc: "A background-checked AllFix pro arrives on schedule, completes the job, and you pay only when satisfied.", step: '3' },
              ].map(({ Icon, title, desc, step }) => (
                <Box key={step} sx={{ flex: 1, zIndex: 2, display: 'flex', flexDirection: 'column', alignItems: 'center' }}>
                  <Box sx={{ position: 'relative', mb: 2 }}>
                    <Box sx={{ width: 70, height: 70, borderRadius: '50%', bgcolor: 'white', boxShadow: '0 2px 12px rgba(16,53,95,0.10)', display: 'flex', alignItems: 'center', justifyContent: 'center', color: '#10355f' }}><Icon sx={{ fontSize: 32 }} /></Box>
                    <Box sx={{ position: 'absolute', top: 0, right: -10, bgcolor: '#10355f', color: 'white', width: 24, height: 24, borderRadius: '50%', display: 'flex', alignItems: 'center', justifyContent: 'center', fontWeight: 700, fontSize: '0.85rem', border: '3px solid #eef4fd' }}>{step}</Box>
                  </Box>
                  <Typography sx={{ fontWeight: 900, fontSize: '1.1rem', mb: 1, color: '#10355f' }}>{title}</Typography>
                  <Typography sx={{ color: '#42526e', fontSize: '0.85rem', textAlign: 'center', maxWidth: 280 }}>{desc}</Typography>
                </Box>
              ))}
            </Box>

            <Box sx={{ textAlign: 'center', mt: 4 }}>
              <Button variant="contained" href="#services" endIcon={<ArrowForwardIcon />} sx={{ bgcolor: '#10355f', color: 'white', fontWeight: 900, fontSize: '0.95rem', px: 3, py: 1, borderRadius: '12px', textTransform: 'none', boxShadow: '0 4px 16px rgba(16,53,95,0.10)', '&:hover': { bgcolor: '#0d264a' } }}>
                Start Booking Now
              </Button>
            </Box>
          </Container>
          ...
          <Box sx={{ width: '100%', maxWidth: 1400, mx: 'auto', px: { xs: 2, md: 6 } }}>
            ...
            <Box sx={{ display: 'flex', flexWrap: 'wrap', justifyContent: 'center', mb: 4 }}>
              <Box sx={{ display: 'flex', flex: 1, minWidth: 290, maxWidth: 1000, bgcolor: '#fff', borderRadius: 3, boxShadow: '0 4px 14px rgba(16,53,95,0.10)', overflow: 'hidden', mx: 'auto' }}>
                {[
                  ...
                  { value: '5,200+',label: 'VERIFIED PROS'    },
                  { value: '4.9★',  label: 'AVERAGE RATING'   },
                  { value: '98%',   label: 'CLIENT SATISFACTION' },
                ].map((stat, idx, arr) => (
                  <Box key={stat.label} sx={{ flex: 1, p: { xs: 1.5, md: 2 }, textAlign: 'center', borderRight: idx < arr.length - 1 ? { xs: 'none', md: '1px solid #e5eaf2' } : 'none' }}>
                    <Typography sx={{ fontWeight: 900, fontSize: { xs: '1.2rem', md: '1.4rem' }, color: '#23406e', mb: 0.4, letterSpacing: '-0.03em' }}>{stat.value}</Typography>
                    <Typography sx={{ color: '#42526e', fontWeight: 700, fontSize: '0.8rem', letterSpacing: '0.05em' }}>{stat.label}</Typography>
                  </Box>
                ))}
              </Box>
            </Box>
            ...
            <Box sx={{ display: 'flex', flexWrap: 'wrap', gap: { xs: 2, md: 2 }, justifyContent: 'center', maxWidth: 1000, mx: 'auto' }}>
              {[
                ...
                <Box key={idx} sx={{ flex: '1 1 280px', minWidth: 260, maxWidth: 320, bgcolor: '#fff', borderRadius: 2, p: 2, mb: { xs: 1.5, md: 1.5 }, boxShadow: '0 1px 6px rgba(16,53,95,0.07)', display: 'flex', gap: 1.5, alignItems: 'flex-start', border: '1px solid #e5eaf2' }}>
            ...
            <Box sx={{ mt: 3, mb: { xs: 6, md: 8 }, width: '100%', bgcolor: '#123865', borderRadius: '20px', display: 'flex', alignItems: 'center', px: { xs: 2, md: 4 }, py: { xs: 2, md: 2.5 }, boxShadow: '0 2px 12px rgba(16,53,95,0.10)', gap: 2, minHeight: 90, maxHeight: 120 }}>
              ...
              <Box sx={{ flex: 1, minWidth: 0 }}>
                ...
                <Typography sx={{ color: 'rgba(255,255,255,0.85)', fontSize: { xs: '0.8rem', md: '0.85rem' }, lineHeight: 1.3 }}>AllFix.ph is officially registered with the Philippine Department of Trade & Industry and compliant with all local labor laws.</Typography>
              </Box>
              <Box sx={{ ml: 2 }}>
        ...
        {/* SERVICE AREA MAP                                                 */}
        {/* ---------------------------------------------------------------- */}
        <Box id="service-area" sx={{ minHeight: '100vh', display: 'flex', flexDirection: 'column', justifyContent: 'center', bgcolor: '#f0f4f8', py: { xs: 8, md: 10 }, position: 'relative', overflow: 'hidden' }}>
          <Container maxWidth="lg" sx={{ textAlign: 'center', position: 'relative', zIndex: 2 }}>
            ...
            <Box sx={{ position: 'relative', width: '100%', maxWidth: { xs: '100%', sm: '550px', md: '650px' }, mx: 'auto', mt: 4, aspectRatio: '5/7' }}>
              {/* SVG map polygons */}
              ...
              <Box sx={{ position: 'absolute', inset: 0, zIndex: 2 }}>
                {mapCities.map((loc, idx) => (
                  <Box
                    key={idx}
                    sx={{ position: 'absolute', top: `${(loc.ly / 700) * 100}%`, left: `${(loc.lx / 500) * 100}%`, transform: 'translate(-50%, -50%)', display: 'flex', flexDirection: 'column', alignItems: 'center', cursor: 'pointer', transition: 'transform 0.2s', '&:hover': { transform: 'translate(-50%, -55%) scale(1.1)', zIndex: 10 } }}
                  >
                    <Box sx={{ width: { xs: 44, md: 54 }, height: { xs: 44, md: 54 }, borderRadius: '50%', bgcolor: 'rgba(255, 255, 255, 0.25)', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                    ...
                    <Typography fontWeight="900" color="#10355f" sx={{ fontSize: { xs: '0.7rem', md: '0.8rem' }, mt: 0.5, bgcolor: '#ffffff', px: 1.2, py: 0.3, borderRadius: '999px', boxShadow: '0 2px 6px rgba(0,0,0,0.15)', whiteSpace: 'nowrap' }}>
                      {loc.id}
                    </Typography>
                  </Box>
                ))}
        ...
        {/* TESTIMONIALS                                                     */}
        {/* ---------------------------------------------------------------- */}
        <Box id="testimonials" sx={{ position: 'relative', left: '50%', right: '50%', ml: '-50vw', mr: '-50vw', width: '100vw', bgcolor: '#0d264a', pt: { xs: 8, md: 10 }, pb: { xs: 8, md: 10 }, display: 'flex', justifyContent: 'center', alignItems: 'center', flexDirection: 'column', overflow: 'hidden' }}>
          <Container maxWidth="xl" sx={{ position: 'relative', zIndex: 10 }}>
            <Box sx={{ textAlign: 'center', mb: 6 }}>
              <Box sx={{ display: 'inline-flex', alignItems: 'center', backgroundColor: '#23406e', color: 'white', borderRadius: '999px', px: 2.5, py: 0.8, fontSize: '0.9rem', fontWeight: 700, letterSpacing: '0.08em', textTransform: 'uppercase', mb: 1.5 }}>CLIENT STORIES</Box>
            ...
            <Box sx={{ display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', mb: 4 }}>
              <Box sx={{ bgcolor: 'white', borderRadius: 3, boxShadow: '0 8px 32px rgba(16,53,95,0.18)', p: { xs: 2.5, md: 4 }, width: { xs: '90vw', md: '700px' }, maxWidth: '700px', minHeight: { xs: 240, md: 180 }, display: 'flex', flexDirection: 'column', justifyContent: 'center', mb: 1.5, position: 'relative' }}>
                <Box sx={{ display: 'flex', alignItems: 'center', gap: 1.5, mb: 1 }}>
                  ...
                  <Box sx={{ textAlign: 'left' }}>
                    <Typography sx={{ fontWeight: 900, color: '#10355f', fontSize: '1rem' }}>{testimonials[testimonialIdx].name}</Typography>
                    <Typography sx={{ color: '#42526e', fontSize: '0.85rem' }}>{testimonials[testimonialIdx].role}</Typography>
                  </Box>
                  <Box sx={{ flex: 1 }} />
                  <Box sx={{ bgcolor: testimonials[testimonialIdx].highlightColor, color: testimonials[testimonialIdx].highlightText, borderRadius: '999px', px: 1.5, py: 0.4, fontWeight: 700, fontSize: '0.85rem' }}>{testimonials[testimonialIdx].highlight}</Box>
                ...
                <Typography sx={{ color: '#222', fontSize: '1rem', fontWeight: 500, mt: 1.5, lineHeight: 1.6 }}>{testimonials[testimonialIdx].text}</Typography>
              </Box>
            ...
            {/* CTA bar */}
            <Box sx={{ position: 'relative', left: '50%', right: '50%', ml: '-50vw', mr: '-50vw', width: '100vw', bgcolor: 'white', py: { xs: 2, md: 2.5 }, px: { xs: 2, md: 6 }, mt: { xs: 5, md: 6 }, display: 'flex', flexDirection: { xs: 'column', md: 'row' }, alignItems: { xs: 'flex-start', md: 'center' }, justifyContent: 'space-between', boxShadow: '0 2px 16px rgba(16,53,95,0.07)' }}>
              <Box sx={{ flex: 1, minWidth: 0 }}>
                <Typography sx={{ fontWeight: 900, fontSize: { xs: '1.4rem', md: '1.5rem' }, color: '#10355f', mb: 0.5 }}>Ready to Fix Something?</Typography>
                <Typography sx={{ color: '#23406e', fontSize: { xs: '0.85rem', md: '0.9rem' }, mb: { xs: 2, md: 0 } }}>Get a free quote in 30 minutes. Available across Metro Manila.</Typography>
              </Box>
              <Box sx={{ display: 'flex', gap: 1.5, mt: { xs: 2, md: 0 } }}>
                ...
                <Button variant="contained" startIcon={<svg width="18" height="18" fill="none" stroke="currentColor" strokeWidth="2" viewBox="0 0 24 24"><path d="M22 16.92V19a2 2 0 0 1-2.18 2A19.72 19.72 0 0 1 3 5.18 2 2 0 0 1 5 3h2.09a2 2 0 0 1 2 1.72c.13 1.05.37 2.07.71 3.06a2 2 0 0 1-.45 2.11l-.27.27a16 16 0 0 0 6.29 6.29l.27-.27a2 2 0 0 1 2.11-.45c.99.34 2.01.58 3.06.71A2 2 0 0 1 22 16.92z"/></svg>} sx={{ bgcolor: '#123865', color: 'white', fontWeight: 800, fontSize: '0.85rem', px: 2.5, py: 1, borderRadius: '10px', boxShadow: 'none', '&:hover': { bgcolor: '#23406e' } }}>Call Us</Button>
              </Box>
            </Box>
          </Container>
        </Box>
            ...
            <Box sx={{ display: 'flex', flexDirection: { xs: 'column', md: 'row' }, alignItems: 'center', justifyContent: 'space-between', gap: { xs: 6, md: 4, lg: 8 }, width: '100%' }}>
              ...
              <Box sx={{ width: { xs: '100%', md: '45%' } }}>
                ...
                <Box sx={{ display: 'flex', flexDirection: 'column', gap: 1.5 }}>
                  {[
                    { label: 'EMAIL US', value: 'inquiry@allfix.ph', svgPath: <path strokeLinecap="round" strokeLinejoin="round" d="M3 8l7.89 5.26a2 2 0 002.22 0L21 8M5 19h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v10a2 2 0 002 2z"/> },
                    { label: 'CALL US',  value: '+63 920 9631 217',   svgPath: <path strokeLinecap="round" strokeLinejoin="round" d="M3 5a2 2 0 012-2h3.28a1 1 0 01.948.684l1.498 4.493a1 1 0 01-.502 1.21l-2.257 1.13a11.042 11.042 0 005.516 5.516l1.13-2.257a1 1 0 011.21-.502l4.493 1.498a1 1 0 01.684.949V19a2 2 0 01-2 2h-1C9.716 21 3 14.284 3 6V5z"/> },
                  ].map(({ label, value, svgPath }) => (
                    <Box key={label} sx={{ bgcolor: 'white', p: 1.5, borderRadius: 3, display: 'flex', alignItems: 'center', gap: 2, boxShadow: '0 4px 12px rgba(16,53,95,0.04)', border: '1px solid #e2e8f0', width: '100%', maxWidth: '340px' }}>
                      <Box sx={{ width: 40, height: 40, borderRadius: '10px', bgcolor: '#eaf2fc', display: 'flex', alignItems: 'center', justifyContent: 'center', color: '#2e5ba8', flexShrink: 0 }}>
                        <svg width="18" height="18" fill="none" stroke="currentColor" strokeWidth="2" viewBox="0 0 24 24">{svgPath}</svg>
                      </Box>
                      <Box>
                        <Typography sx={{ fontSize: '0.7rem', fontWeight: 700, color: '#64748b', letterSpacing: '0.05em' }}>{label}</Typography>
                        <Typography sx={{ fontSize: '0.95rem', fontWeight: 800, color: '#10355f' }}>{value}</Typography>
                      </Box>
                    </Box>
                  ))}
                </Box>
              ...
              <Box sx={{ width: { xs: '100%', md: '50%' } }}>
                <Box sx={{ bgcolor: '#ffffff', borderRadius: '20px', p: { xs: 3, md: 4 }, boxShadow: '0 20px 40px rgba(16,53,95,0.08)', border: '1px solid #eaf2fc' }}>
                  <Typography sx={{ fontSize: { xs: '1.2rem', md: '1.4rem' }, fontWeight: 900, color: '#10355f', mb: 0.5, letterSpacing: '-0.01em' }}>Send a direct message</Typography>
                  ...
                  <Grid container spacing={2.5}>
                    <Grid item xs={12} sm={6}>
                      ...
                      <TextField size="small" fullWidth placeholder="e.g. Juan Dela Cruz" sx={{ '& .MuiOutlinedInput-root': { bgcolor: '#f8fafc', borderRadius: '8px', fontSize: '0.85rem', '& fieldset': { borderColor: '#e2e8f0' }, '&:hover fieldset': { borderColor: '#cbd5e1' }, '&.Mui-focused fieldset': { borderColor: '#2e5ba8', borderWidth: '2px' } } }} />
                    </Grid>
                    <Grid item xs={12} sm={6}>
                      <Typography sx={{ fontSize: '0.75rem', fontWeight: 700, color: '#10355f', mb: 0.8 }}>Email Address</Typography>
                      <TextField size="small" fullWidth placeholder="juan@email.com" sx={{ '& .MuiOutlinedInput-root': { bgcolor: '#f8fafc', borderRadius: '8px', fontSize: '0.85rem', '& fieldset': { borderColor: '#e2e8f0' }, '&:hover fieldset': { borderColor: '#cbd5e1' }, '&.Mui-focused fieldset': { borderColor: '#2e5ba8', borderWidth: '2px' } } }} />
                    </Grid>
                    <Grid item xs={12}>
                      <Typography sx={{ fontSize: '0.75rem', fontWeight: 700, color: '#10355f', mb: 0.8 }}>How can we help?</Typography>
                      <TextField fullWidth multiline rows={4} placeholder="Tell us about your concern..." sx={{ '& .MuiOutlinedInput-root': { bgcolor: '#f8fafc', borderRadius: '8px', fontSize: '0.85rem', '& fieldset': { borderColor: '#e2e8f0' }, '&:hover fieldset': { borderColor: '#cbd5e1' }, '&.Mui-focused fieldset': { borderColor: '#2e5ba8', borderWidth: '2px' } } }} />
                    </Grid>
                  </Grid>
                  <Button fullWidth variant="contained" endIcon={<ArrowForwardIcon />} sx={{ mt: 3.5, py: 1.2, bgcolor: '#10355f', color: 'white', borderRadius: '8px', fontWeight: 800, fontSize: '0.9rem', textTransform: 'none', boxShadow: '0 4px 12px rgba(16,53,95,0.2)', '&:hover': { bgcolor: '#0d264a', transform: 'translateY(-2px)', boxShadow: '0 6px 16px rgba(16,53,95,0.3)' } }}>
                    Submit Message
                  </Button>
                </Box>
              </Box>
          ...
          <Container maxWidth="xl" sx={{ px: { xs: 3, md: 6 } }}>
            <Grid container spacing={{ xs: 4, md: 10, lg: 12 }}>
              ...
              <Grid item xs={12} md={4}>
                ...
                <Box sx={{ display: 'flex', gap: 1.5 }}>
                  {[
                    ...
                    <IconButton key={i} sx={{ border: '1px solid rgba(255,255,255,0.15)', borderRadius: '8px', color: 'rgba(255,255,255,0.7)', '&:hover': { color: 'white', bgcolor: 'rgba(255,255,255,0.1)' } }}>
                      <svg viewBox="0 0 24 24" width="18" height="18" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">{icon}</svg>
                    </IconButton>
                  ))}
                </Box>
              </Grid>
              ...
              <Grid item xs={6} md={2}>
                ...
                <Box sx={{ display: 'flex', flexDirection: 'column', gap: 2 }}>
                  {['About AllFix', 'Careers', 'Press & Media', 'Investor Relations'].map((link) => (
                    <Typography key={link} onClick={() => { if (link === 'About AllFix') { navigate('/about'); } }} sx={{ color: 'rgba(255,255,255,0.6)', fontSize: '0.9rem', cursor: 'pointer', '&:hover': { color: 'white' } }}>{link}</Typography>
                  ))}
                </Box>
              </Grid>
              ...
              <Grid item xs={6} md={2}>
                ...
                <Box sx={{ display: 'flex', flexDirection: 'column', gap: 2 }}>
                  {['Air-con (CoolFix)', 'Cleaning (SaniFix)', 'Repairs (HomeFix)', 'IT Support (TechFix)', 'Moving (MoveFix)', 'Health (HealthFix)', 'Eco (GreenFix)', 'Pets (PetFix)'].map((link) => (
              ...
              <Grid item xs={6} md={2}>
                ...
                <Box sx={{ display: 'flex', flexDirection: 'column', gap: 2 }}>
                  {['Help Center', 'Book a Service', 'Track My Job', 'Partner With Us'].map((link) => (
                    <Typography key={link} onClick={() => { if (link === 'Partner With Us') { navigate('/vendor-apply'); window.scrollTo(0, 0); } }} sx={{ color: 'rgba(255,255,255,0.6)', fontSize: '0.9rem', cursor: 'pointer', '&:hover': { color: 'white' } }}>{link}</Typography>
                  ))}
                </Box>
              </Grid>
            ...
            <Grid container spacing={4} justifyContent="space-between" alignItems="center">
              {[
                { icon: <path d="M22 16.92v3a2 2 0 0 1-2.18 2 19.79 19.79 0 0 1-8.63-3.07 19.5 19.5 0 0 1-6-6 19.79 19.79 0 0 1-3.07-8.67A2 2 0 0 1 4.11 2h3a2 2 0 0 1 2 1.72 12.84 12.84 0 0 0 .7 2.81 2 2 0 0 1-.45 2.11L8.09 9.91a16 16 0 0 0 6 6l1.27-1.27a2 2 0 0 1 2.11-.45 12.84 12.84 0 0 0 2.81.7A2 2 0 0 1 22 16.92z"/>, label: 'CALL US',     value: '+63 920 9631 217 | +63 975 8336 289' },
                { icon: <><path d="M4 4h16c1.1 0 2 .9 2 2v12c0 1.1-.9 2-2 2H4c-1.1 0-2-.9-2-2V6c0-1.1.9-2 2-2z"/><polyline points="22,6 12,13 2,6"/></>, label: 'EMAIL US',    value: 'inquiry@allfix.ph' },
              ].map(({ icon, label, value }) => (
                <Grid item xs={12} md={4} key={label}>
                  <Box sx={{ display: 'flex', alignItems: 'flex-start', gap: 2 }}>
                    <Box sx={{ mt: 0.5, color: 'white' }}><svg viewBox="0 0 24 24" width="22" height="22" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">{icon}</svg></Box>
                    <Box>
                      <Typography sx={{ fontSize: '0.75rem', fontWeight: 700, color: 'rgba(255,255,255,0.9)', mb: 0.3, letterSpacing: '0.05em' }}>{label}</Typography>
                      <Typography sx={{ fontSize: '1.05rem', fontWeight: 700, color: 'white' }}>{value}</Typography>
                    </Box>
                  </Box>
                </Grid>
              ))}
