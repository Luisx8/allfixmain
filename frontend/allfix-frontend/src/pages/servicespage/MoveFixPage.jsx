...
const movefixServices = [
  { title: 'Full-Service Packing', description: 'Supplying boxes, bubble wrap, and expertly packing fragile items prior to moving day.', tag: 'Packing', image: 'https://images.unsplash.com/photo-1600585154340-be6161a56a0c?w=400&h=400&fit=crop' },
  { title: 'Loading & Transport', description: 'Safe lifting and secure transportation via closed vans or trucks.', tag: 'Logistics', image: 'https://images.unsplash.com/photo-1586864387967-d02ef85d93e8?w=400&h=400&fit=crop' },
  { title: 'Appliance Relocation', description: 'Specialized handling for heavy/delicate items like refrigerators, washing machines, and large TVs.', tag: 'Heavy Lifting', image: 'https://images.unsplash.com/photo-1581578731548-c64695cc6952?w=400&h=400&fit=crop' },
  { title: 'Unpacking & Setup', description: 'Helping clients unbox items and place them in their designated rooms.', tag: 'Setup', image: 'https://images.unsplash.com/photo-1505691938895-1758d7feb511?w=400&h=400&fit=crop' },
  { title: 'Storage Solutions', description: 'Short-term storage box rentals for items during home staging or delayed move-ins.', tag: 'Storage', image: 'https://images.unsplash.com/photo-1616401784845-180882ba9ba8?w=400&h=400&fit=crop' },
  { title: 'Professional Moving', description: 'Complete household and commercial moving services with careful handling.', tag: 'Relocation', image: 'https://images.unsplash.com/photo-1603796846027-ea4a052e4f30?w=400&h=400&fit=crop' },
];
          ...
          <Container maxWidth="md" sx={{ position: 'relative', zIndex: 10, textAlign: 'center' }}>
            <Box sx={{ display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 2, mb: 1 }}>
              <Box sx={{ display: 'flex', alignItems: 'center', justifyContent: 'center', bgcolor: 'rgba(255,255,255,0.1)', borderRadius: '12px', p: 1 }}>
                <LocalShippingIcon sx={{ fontSize: { xs: '2rem', md: '2.5rem' }, color: '#ffffff' }} />
              </Box>
              <Typography variant="h1" fontWeight="900" sx={{ fontSize: { xs: '2.2rem', md: '3.2rem' }, color: 'white', letterSpacing: '-0.02em', m: 0 }}>
                Move<span style={{ color: '#017550' }}>F</span><span style={{ color: '#fcbc26' }}>i</span><span style={{ color: '#d8242b' }}>x</span>
              </Typography>
            </Box>
            <Typography sx={{ fontSize: { xs: '0.95rem', md: '1.1rem' }, lineHeight: 1.4, maxWidth: '700px', mx: 'auto', color: 'rgba(255,255,255,0.8)' }}>
              Professional Moving Solutions | Reliable • Efficient • Careful
            </Typography>
          </Container>
        </Box>
          ...
          <Container maxWidth="lg">
                ...
                {footerPills.map(pill => (
                  <Box
                    ...
                    sx={{
                      ...
                      px: { xs: 1.5, md: 2 },
                      py: { xs: 0.6, md: 0.8 },
                      bgcolor: pill.name === 'MoveFix' ? '#10355f' : '#ffffff',
                      color: pill.name === 'MoveFix' ? 'white' : '#334155',
                      border: '1px solid',
                      borderColor: pill.name === 'MoveFix' ? '#10355f' : '#e2e8f0',
                      borderRadius: '50px',
                      cursor: 'pointer',
                      transition: 'all 0.3s ease',
                      boxShadow: pill.name === 'MoveFix' ? '0 8px 16px rgb(255, 255, 255)' : '0 2px 4px rgba(0,0,0,0.04)',
                      '&:hover': {
                        bgcolor: pill.name === 'MoveFix' ? '#10355f' : '#f8fafc',
                        borderColor: pill.name === 'MoveFix' ? '#10355f' : '#cbd5e1',
            ...
            <Box sx={{ mb: 10, display: 'grid', gridTemplateColumns: { xs: '1fr', md: '1fr 1fr' }, gap: { xs: 4, md: 6 }, alignItems: 'center' }}>
              <Box sx={{ position: 'relative', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                <Box sx={{
                  ...
                  borderRadius: '16px',
                  overflow: 'hidden',
                  boxShadow: '0 16px 32px rgba(255, 111, 0, 0.15)',
                  border: '4px solid #fff3e0'
                }}>
                  <Box
                    component="img"
                    src="/images/movefix.jpg"
                    alt="MoveFix"
              ...
              <Box>
                <Typography variant="h2" sx={{ fontSize: { xs: '1.8rem', md: '2.2rem' }, fontWeight: 900, color: '#10355f', mb: 2, lineHeight: 1.2 }}>
                  About Move<span style={{ color: '#017550' }}>F</span><span style={{ color: '#fcbc26' }}>i</span><span style={{ color: '#d8242b' }}>x</span>
                </Typography>
                <Typography sx={{ mb: 3, color: '#666', lineHeight: 1.8, fontSize: '1rem' }}>
                  MoveFix is AllFix.ph's specialized moving and relocation division. We provide complete moving solutions for residential and commercial customers across Metro Manila. Whether you're moving across town or across the city, we ensure a smooth, stress-free transition.
                </Typography>
                <Typography sx={{ mb: 4, color: '#666', lineHeight: 1.8, fontSize: '1rem' }}>
                  With professional movers, quality equipment, and proven logistics expertise, we handle your move with care and efficiency. Book our team today to take the heavy lifting off your shoulders.
                </Typography>
              </Box>
            </Box>
            ...
            <Box sx={{ mb: 10 }}>
              ...
              <Box sx={{
                ...
                {movefixServices.map((service, index) => (
                  <Box key={index} sx={{
                    display: 'flex',
                    ...
                    '&:hover': {
                      ...
                      boxShadow: '0 16px 32px rgba(255, 111, 0, 0.1)',
                    }
                  }}>
            ...
            <Box sx={{
              ...
              <Box sx={{ position: 'absolute', top: -60, right: -60, width: 240, height: 240, borderRadius: '50%', bgcolor: 'rgba(255, 179, 0, 0.08)', pointerEvents: 'none' }} />
              <Box sx={{ position: 'absolute', bottom: -40, left: -40, width: 180, height: 180, borderRadius: '50%', bgcolor: 'rgba(255, 111, 0, 0.07)', pointerEvents: 'none' }} />
              <Box sx={{ position: 'relative', zIndex: 1, p: { xs: 4, md: 6 }, display: 'flex', flexDirection: { xs: 'column', md: 'row' }, alignItems: 'center', justifyContent: 'space-between', gap: 4 }}>
                <Box sx={{ flex: 1 }}>
                  <Typography variant="h3" sx={{ fontSize: { xs: '1.6rem', md: '2.2rem' }, fontWeight: 900, color: 'white', lineHeight: 1.15, mb: 1.5 }}>Ready to Relocate?</Typography>
                  <Typography sx={{ color: 'rgba(255,255,255,0.65)', fontSize: '0.97rem', lineHeight: 1.7, maxWidth: 460 }}>Book a MoveFix team today. Experience a hassle-free relocation with our professional movers. We handle your belongings with the utmost care.</Typography>
                </Box>
